...
深度学习已应用于核截面预测：DINo算法引入了处理聚变相关核素复杂共振结构的新架构[84]。物理信息神经网络已应用于具有非均匀系数的中子扩散方程[85]。

### 6.6 ML用于氚行为预测

ML势函数分子动力学模拟已用于研究氢同位素与钨表面在等离子体相关能量下的相互作用[87]。TMAP8框架中的代理模型实现了聚变电站设计中氚库存和渗透的多尺度评估[88]。ML还已应用于ITER DT聚变功率的伽马射线光谱测量[89]。
