...
AgentRuntimeFabric, 以下简称 ARF, 是开源、模型无关、可自托管的 Agent Runtime Fabric。它位于 Agent SDK/Harness 与 sandbox/runtime 后端之间，为长时运行、会修改代码仓库的 Agent 提供可恢复、可审计、受策略约束、后端可替换的控制平面。
...
- 开源生态仍缺少一个把 workflow、workspace lineage、policy lease、runtime adapter、ChangeSet、EvidenceGraph 和 replay 统一成开放 schema 的自托管控制平面。
...
本说明书采用架构评审视角组织，而非实现任务清单。文档先定义目的、概念模型、质量属性和架构原则，再给出系统用例、关键技术方案，以及逻辑模型、技术模型、数据模型、代码模型、构建模型和部署模型。
...
ARF 要解决的问题是：当前大量 coding agent 和 agentic workflow 能够调用工具、运行命令、修改文件，但在长任务、失败恢复、权限治理、证据链、多人协作和后端替换方面缺少统一平台边界。典型问题包括：
...
- 用 `RuntimeAdapter` 和 `RuntimeCapabilities` 接入异构 runtime 后端。
...
| 章节 | 内容 |
| --- | --- |
| 1. 简介 | 目的、范围、文档结构、已有架构借鉴和反思 |
| 2. 概念模型 | 领域概念、对象关系、生命周期、不变式和能力地图 |
| 3. 架构和关键质量属性目标 | 架构目标、关键需求、质量属性、假设与约束 |
| 4. 架构原则 | 战略性原则和结构性原则 |
| 5. 系统用例模型 | 上下文模型、上下文图、外部接口、关键系统用例 |
| 6. 关键技术方案 | Workflow、workspace、policy、runtime、evidence、governance、replay 等关键方案 |
| 7. 系统架构设计模型 | 逻辑模型、技术模型、数据模型、代码模型、构建模型、部署模型 |
| 8. 架构治理 | ADR、架构适应度函数、发布闸门、开放问题 |
| 9. 结论 | 评审结论和 MVP 成功标准 |
...
| 托管企业 Agent 平台 | Runtime、Memory、Gateway、Identity、Observability、Policy、Registry 的完整产品形态 | ARF 不应宣称“企业平台能力独有”，而应提供开源、自托管、schema 可检查、后端可替换的替代控制面 |
...
2. ARF 必须把 runtime 后端当作可替换资源，而不是事实源。
...
| 后端可替换 | container、gVisor、microVM、remote sandbox、browser、GPU 后端能力不同 | `RuntimeAdapter`, `RuntimeInstance`, `RuntimeCapabilities`, `AdapterMapping` |
...
| `RuntimeCapabilities` | 后端能力声明 | `backend`, `isolation_class`, `snapshot_classes`, `supports_reconnect`, `port_modes`, `limits` |
...
| 执行资源层 | `RuntimeInstance`, `RuntimeCapabilities`, `AdapterMapping` | Runtime service / Adapter registry | 部分是 | 哪个 runtime 后端执行，能力是什么，如何防止 vendor schema 泄漏 |
...
| 上下文复用层 | `ContextPack`, `KnowledgeMount`, `Skill` | Semantic Context / SkillRegistry | 否，属于事实投影 | 如何把事实压缩给模型使用，但不扩大权限或替代证据 |
...
6. `ContextPack`、summary、embedding、Skill suggestion 都是投影，不能覆盖事实和策略。
...
| 状态 | 目标 | 关键事件 |
| --- | --- | --- |
| `Allocating/Booting` | 获得后端资源并启动环境 | `RuntimeAllocated`, `RuntimeBooted` |
...
| `Hibernating` | 空闲降本或后端 pause | `RuntimePaused/Resumed` |
| `Lost` | 心跳丢失、OOM、抢占 | `RuntimeLost` |
| `Reclaimed` | idle TTL 或 operator 回收 | `RuntimeReclaimed` |

#### 2.7.6 WorkspaceBranch 与 Merge 状态机
...
#### 2.7.9 Adapter Admission 状态机

目标：控制外部 runtime/sandbox/workflow/agent 后端接入，确保能力可验证、schema 不污染核心模型、许可证和安全风险可控。

原则：

- 没有 Mapping ADR 的 adapter 不能进入 registry。
- 没有通过 contract tests 的 adapter 不能参与路由。
- 远程/闭源/云 backend 可以作为 backend，但不能成为 ARF 控制面事实源。
...
| 状态 | 目标 | 准入证据 |
| --- | --- | --- |
| `Proposed` | 提交后端接入意图 | owner、backend、use case |
...
| 后端可替换 | Docker/gVisor/E2B/Modal/Daytona/Firecracker/OpenHands 等作为 adapter 后端 | fake + container adapter 通过同一 contract tests |
...
| AR-07 | 多后端 runtime 不污染核心模型 | RuntimeCapabilities、AdapterMapping ADR、schema lint、contract tests |
...
| 可演进性 | 引入 OPA/Cedar/Vault/Temporal | 对外仍输出 PolicyDecision、SecretGrant、WorkflowRun，不泄漏后端 schema |
...
| 外部后端 | 云 sandbox、workflow engine、policy engine 只能作为 adapter 或实现后端，不能成为 ARF 事实源 |
...
4. 可观测和可运维。
5. 性能和成本优化。
6. 多后端、多 Agent、Knowledge/Skill 产品化。
...
| 原则 | 说明 | 影响 |
| --- | --- | --- |
| 开源控制面优先 | ARF 的核心价值是开放 schema、自托管和可替换后端 | 不能依赖云账号或闭源控制面才能跑通 MVP |
...
ARF 位于用户入口、Agent 编排层、执行后端和治理系统之间。
...
### 6.2 Workspace Lineage 与 Snapshot
...
### 6.4 RuntimeAdapter 与防腐层

RuntimeAdapter 是 ARF 接入执行后端的唯一长期契约。
...
### 6.8 Idempotency、Effect Ledger 与补偿
...
Execution Plane 只执行已授权动作。RuntimeAdapter 负责屏蔽后端差异，SandboxDaemon 负责在执行面强制校验 lease。

#### 7.1.7 State Plane 架构图
...
#### 7.1.8 Evidence / Observability Plane 架构图
...
#### 7.1.9 Semantic Context Plane 架构图
...
| Adapter Admission Protocol | Adapter author -> Registry | `RuntimeCapabilities`, `AdapterMapping`, `ContractResult` | 后端准入和防腐 | vendor schema 进入 public API |

#### 7.2.2 Control API 协议图
...
#### 7.2.3 Workflow Command + Event Protocol 图
...
#### 7.2.9 Adapter Admission Protocol 图
...
| `runtime_capabilities` | `adapter_id` | 后端能力声明 |
...
- ADR required: 新公共对象、新协议、新后端准入必须有 ADR。
...
| ADR-010 | Adapter capability matrix 允许后端差异，但禁止 vendor schema 泄漏 | 必做 |
...
最终评审结论：ARF 应以“开源、自托管、可检查、可替换的 code-changing agent change-control runtime fabric”为长期定位。它的架构核心是 `WorkflowRun / Task / ExecutionLease / RuntimeInstance` 四层生命周期、`Workspace / Snapshot / ChangeSet` 工程现场模型、`PolicyDecision / ExecutionLease / Approval / SecretGrant` 治理模型、`RuntimeAdapter / RuntimeCapabilities` 后端防腐模型，以及 `EventLog / Artifact / EvidenceGraph / Replay` 证据模型。只要这些模型保持清晰并以 contract tests 和 recovery demo 证明，ARF 才能支撑国际顶级专家评审和后续开源实现。
