# 华为云 ModelArts 训推平台：业务背景、技术架构、实践与改进路径
...
传统深度学习时代的 MoXing/AutoSearch 范式向大模型迁移的过渡未完成、MoE 系统软件在昇腾上的成熟度不足、生态封闭性限制开发者基数、可复现性基线缺失。提出七个改进方向，改进优先级矩阵如下（**注：预期吞吐收益为借用 NVIDIA 生态学术前沿的迁移估算，未经昇腾 NPU 实测验证；工程投入经 §7.8 接口可用性评估修正**）：
