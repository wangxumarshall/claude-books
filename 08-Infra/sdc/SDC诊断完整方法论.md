...
> **基于论文**：ASPLOS 2025 Hardware Sentinel (Meta) / HPCA 2026 PinDrop (Meta/MIT) / SOSP 2023 Alibaba / HPCA 2025 Veritas / ASPLOS 2026 SEVI / MICRO 2024 DelayAVF / SOSP 2025 Orthrus / ISCA 2024 Harpocrates / Cross-ISA SDC / Differential FI / Gem5-MARVEL / Soft Error Effects on ARM
...
| 维度 | HWSentinel | Fleetscanner | Ripple | PinDrop |
|------|-----------|-------------|--------|---------|
| **检测方式** | 应用层异常分析（top-down） | 离线测试（out-of-production） | 在线测试（in-production） | 连续测试表征（continuous） |
| **运行时机** | 离线持续分析 | 维护窗口期间 | 与生产工作负载共存 | 定期重访（≤30天） |
| **测试时间** | 数据分析（秒级） | 分钟级 | 毫秒级 | 分钟级 |
| **ARM64测试覆盖** | 所有异常类型 | NEON/SVE指令测试 | 轻量级向量检查 | 9大测试家族全覆盖 |
| **硬件依赖** | 无（vendor-agnostic） | 需要ARM64测试程序 | 需要ARM64测试程序 | 需要ARM64测试程序 |
| **性能开销** | 数据采集开销 | 维护窗口占用 | 与生产负载共享资源 | 维护窗口占用 |
| **核心优势** | 无测试开销，架构无关 | 离线深度测试 | 在线持续检测 | 长期趋势分析，表征规模最大 |
| **论文依据** | ASPLOS 2025 | arXiv 2022 | arXiv 2022 | HPCA 2026 |
...
SDC诊断是一个**多维度、多阶段、持续迭代**的过程。基于Hardware Sentinel框架与PinDrop连续测试框架的ARM64鲲鹏适配版方法论可总结为：
